  ...
  return (
    <div className="w-full h-20 border-b-[1px] border-b-black font-titleFont sticky top-0 bg-white z-50 px-4">
      <div className="max-w-7xl h-full mx-auto flex justify-between items-center">
        <Link href="/">
          <div>
            <Image width={180} height={180} src={logoDark} alt="logoDark" />
          </div>
        </Link>
        <div>
          <ul className="hidden lg:inline-flex gap-8 uppercase text-sm font-semibold">
            <li className="headerLi"><Link href="/">Home</Link></li>
            <li className="headerLi">post</li>
        ...
        <div className="flex items-center gap-8 text-lg">
          <div className="flex items-center gap-1">
            ...
            <img
              ...
              src={session && session.user && session.user.image ? session.user.image : "https://itsking.netlify.app/static/media/Hero.540fddaef6975d508300.webp"}
              alt="logo"
            />


            <p className="text-sm font-medium">
              {session ? session?.user!.name : "Hello Blogers"}
            </p>
          </div>
          {
